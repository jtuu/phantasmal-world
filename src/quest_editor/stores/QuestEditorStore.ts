...
import { MessageLogStore, LogMessage, LogLevel, LogGroup } from "../gui/MessageLogView";
...
export class QuestEditorStore implements Disposable, MessageLogStore {
    ...
    constructor() {
        this.disposer.add_all(
            ...
            this.log_level.observe(() => (this._log_messages.val = this._log_messages.val)),
            this.log_group.observe(() => (this._log_messages.val = this._log_messages.val)),
        );
    ...
    private log_message_predicate = (msg: LogMessage): boolean => {
        return (
            msg.log_group.key === this.log_group.val.key &&
    ...
    public log(...items: any[]): void {
        this.add_log_message(items, this.log_levels_map.Info, this.default_log_group);
    }
}
